...
function registerCustomCommands(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    Commands.registerCommand("lua.reloadFFIMeta", async () => {
      defaultClient?.client?.sendRequest(ExecuteCommandRequest.type, {
  ...
  context.subscriptions.push(
    Commands.registerCommand(
      "lua.showReferences",
      (uri: string, position: Record<string, number>, locations: any[]) => {
  ...
  private provideHover() {
    ...
    let provider = vscode.languages.registerHoverProvider("lua", {
      provideHover: async (
        ...
        return client?.sendRequest(LSP.HoverRequest.type, params, token).then(
          (result) => {
